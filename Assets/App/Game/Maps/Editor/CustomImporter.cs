...
namespace xyz._8bITProject.cooperace {
	...
	class CustomImporter : ICustomTiledImporter {
		...
		/// index to ensure unique object names
		int i = 0;
		...
		GameObject replaceMarker(GameObject marker, GameObject prefab) {
			...
			// assign the object a unique name

			newObject.name = newObject.name + "__" + (i++);
		...
		public void CustomizePrefab(GameObject mapPrefab) {
			...
			// set any collision objects' unity layers to 'Collision'
		...
		void BuildExits(GameObject mapPrefab){
			...
			NewExit(exits, "left", 0 - padding - thickness/2, -height/2,
				thickness, height + 2 * padding + 2 * thickness);

			NewExit(exits, "right", width + padding + thickness/2, -height/2,
				thickness, height + 2 * padding + 2 * thickness);

			NewExit(exits, "above", width/2, 0 + padding + thickness/2,
				width + 2 * padding + 2 * thickness, thickness);

			NewExit(exits, "below", width/2, -height - padding - thickness/2,
				width + 2 * padding + 2 * thickness, thickness);
